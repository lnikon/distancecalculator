...
template <typename ValueType> using Row = std::vector<ValueType>;

template <typename ValueType> using RowSPtr = std::shared_ptr<Row<ValueType>>;
...
template <typename ValueType>
void CSVContainer<ValueType>::resize(const std::size_t rowDim,
                                     const std::size_t) {
  assert(rowDim > 0);
  //    assert(colDim > 0);
 ...
 */
template <typename ValueType>
typename std::vector<RowSPtr<ValueType>>::iterator
CSVContainer<ValueType>::begin() {
  return m_data.begin();
}

template <typename ValueType>
typename std::vector<RowSPtr<ValueType>>::iterator
CSVContainer<ValueType>::end() {
  return m_data.end();
}

template <typename ValueType>
typename std::vector<RowSPtr<ValueType>>::const_iterator
CSVContainer<ValueType>::cbegin() {
  return m_data.cbegin();
}

template <typename ValueType>
typename std::vector<RowSPtr<ValueType>>::const_iterator
CSVContainer<ValueType>::cend() {
  return m_data.cend();
}
...
template <typename ValueType>
typename std::vector<RowSPtr<ValueType>>::iterator
begin(CSVContainer<ValueType> &c) {
  return c.begin();
}

template <typename ValueType>
typename std::vector<RowSPtr<ValueType>>::iterator
end(CSVContainer<ValueType> &c) {
  return c.end();
}

template <typename ValueType>
typename std::vector<RowSPtr<ValueType>>::const_iterator
cbegin(CSVContainer<ValueType> &c) {
  return c.cbegin();
}

template <typename ValueType>
typename std::vector<RowSPtr<ValueType>>::const_iterator
cend(CSVContainer<ValueType> &c) {
  return c.cend();
}
