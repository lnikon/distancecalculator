...
class SequentialDistanceCalculatorEngine
...
public:
  ...
  calculate(structures::CSVContainerSPtr<ValueType> query,
    ...
    for (std::size_t rowIdx = 0; rowIdx < queryRowCount; rowIdx++) {
      for (std::size_t datasetRowIdx = 0; datasetRowIdx < datasetRowCount;
           datasetRowIdx++) {
        ...
        for (std::size_t colIdx = 0; colIdx < queryColumnCount; colIdx++) {
          (*resultRow)[colIdx] = (*dataset->get(datasetRowIdx))[colIdx] -
                                 (*query->get(rowIdx))[colIdx];
        }
